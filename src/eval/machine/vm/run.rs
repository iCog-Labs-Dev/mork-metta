...
use crate::eval::{
    forms::{
        control::is_truthy,
        query::{lookup_user_clauses, match_clause},
    },
    io::{eval_readln, finish_println, load_metta_file, resolve_import_path},
    machine::{
        budget::{ResultSet, calculate_cost, calculate_expr_cost, plain},
        state::unify,
        step::run_rs,
        vm::state::next_fresh_id,
    },
    ...
    shared::{
        ...
        subst::{subst_and_atomize, subst_atom},
    },
};
...
fn run_vm_inner(
    ...
    loop {
        while state.ip < state.code.len() {
            ...
            if debug_vm {
                eprintln!(
                    "IP: {:03} | OP: {:?} | STACK: {:?} | LOCALS: {:?}",
                    state.ip, op, state.stack, state.locals
                );
            }
            match op {
                Opcode::Const(atom) => {
                ...
                Opcode::Load(idx) => {
                    ...
                    let resolved = match &val {
                        Atom::Sym(s) if s.starts_with('$') => lookup(&env, s)
                            .or_else(|| lookup(&base_env, s))
                            .unwrap_or(val),
                        _ => val,
                    };
                ...
                Opcode::Store(idx) => {
                    ...
                    if let Some((atom, env)) = rs.pop() {
                        if state.locals.len() <= *idx as usize {
                            state
                                .locals
                                .resize(*idx as usize + 1, (Atom::sym("()"), Env::new()));
                        }
                ...
                Opcode::LoadFree(idx) => {
                    ...
                    let resolved = match &fresh {
                        Atom::Sym(s) if s.starts_with('$') => lookup(&base_env, s).unwrap_or(fresh),
                        _ => fresh,
                    };
                ...
                Opcode::JumpIfEmpty(target) => {
                    let top = state
                        .stack
                        .pop()
                        .ok_or("VM stack underflow on JumpIfEmpty")?;
                    if top.is_empty() {
                ...
                Opcode::JumpIfFalsy(target) => {
                    let top = state
                        .stack
                        .pop()
                        .ok_or("VM stack underflow on JumpIfFalsy")?;
                ...
                Opcode::UnifyPattern(pattern, start_idx) => {
                    let val_rs = state
                        .stack
                        .pop()
                        .ok_or("VM stack underflow on UnifyPattern")?;
                    if val_rs.is_empty() {
                        state.stack.push(Vec::new());
                        state.ip += 1;
                        continue;
                    ...
                    if let Some((value, _env)) = val_rs.first() {
                        if let Ok(Some(matched_env)) =
                            try_match_one(pattern, value, &Env::new(), funcs)
                        {
                ...
                Opcode::AddAtom { expr, local_names } => {
                    let space_rs = state
                        .stack
                        .pop()
                        .ok_or("VM stack underflow on AddAtom space")?;

                    let mut current_env = base_env.clone();
                    for (i, name) in local_names.iter().enumerate() {
                        if let Some((val, _val_env)) = state.locals.get(i) {
                            current_env =
                                prepend_chain(bind(&Env::new(), name, val.clone()), &current_env);
                        }
                    }

                    let atom = subst_and_atomize(expr, &current_env);
                    let cost = calculate_cost(&atom).unwrap_or(0);
                ...
                Opcode::RemAtom { expr, local_names } => {
                    let space_rs = state
                        .stack
                        .pop()
                        .ok_or("VM stack underflow on RemAtom space")?;
                    ...
                    if let Some((space_ref, _)) = space_rs.first() {
                        let removed = remove_atom(funcs, space_ref, &atom)?;
                ...
                Opcode::Lambda {
                    ...
                    state
                        .stack
                        .push(plain(vec![Atom::Closure(Box::new(ClosureData {
                            params: params.clone(),
                            body: body.clone(),
                            env: current_env,
                        }))]));
                ...
                Opcode::Unify {
                    ...
                    enum UnifyIter {
                        Space(std::vec::IntoIter<MatchResult>),
                        Vals(std::vec::IntoIter<(Atom, Env)>, Option<Atom>),
                        None,
                    }

                    struct UnifyResume {
                        results: Vec<(Atom, Env)>,
                        iter: UnifyIter,
                        current_env: Env,
                    ...
                    let mut resume = match state
                        .resume_data
                        .take()
                        .and_then(|r| r.downcast::<UnifyResume>().ok())
                    {
                        Some(mut r) => {
                        ...
                        None => {
                            let val_b_rs =
                                state.stack.pop().ok_or("VM stack underflow on Unify B")?;
                            let val_a_rs =
                                state.stack.pop().ok_or("VM stack underflow on Unify A")?;

                            let mut current_env = base_env.clone();
                            for (i, name) in local_names.iter().enumerate() {
                                if let Some((val, _)) = state.locals.get(i) {
                                    current_env = prepend_chain(
                                        bind(&Env::new(), name, val.clone()),
                                        &current_env,
                                    );
                                }
                            }
                            ...
                            let precomputed_bindings: Vec<Atom> = free_vars_map
                                ...
                                .map(|name| {
                                    if let Some(pos) =
                                        state.free_vars_map.iter().position(|x| x == name)
                                    {
                                        state.free_vars_bindings[pos].clone()
                                    } else if is_generated_var_name(name) {
                                        Atom::sym(name)
                                    } else {
                            ...
                            let is_space =
                                matches!(first_a, Some(Atom::Sym(s)) if s.starts_with('&'));

                            let (iter, emit_else) = if is_space {
                                let space_ref = first_a.unwrap();
                                let matches = collect_match_results(
                                    funcs,
                                    space_ref,
                                    pattern_b,
                                    &current_env,
                        ...
                        match &mut resume.iter {
                            UnifyIter::Space(iter) => {
                                if let Some(m) = iter.next() {
                                    ...
                                    let sub_env = bind_all(&resume.current_env, &m.bindings);
                                ...
                                if let Some(_) = val_a_opt {
                                    while let Some((val_b, env_b)) = iter.next() {
                                        ...
                                        if let Some(matched_env) =
                                            try_match_one(pattern_a, &val_b, &match_env, funcs)?
                                        {
                                            resume.matched_any = true;
                                            found = true;
                                            let mut sub_locals = state.locals.clone();
                                            for var in pattern_vars {
                                                let bound =
                                                    matched_env.get(var).unwrap_or(Atom::sym("()"));
                                                sub_locals.push((bound, Env::new()));
                                            }
                                            let sub_state = VMState {
                                                code: then_code.clone(),
                ...
                Opcode::Collapse => {
                    let _profile = if cfg!(feature = "profile") {
                        Some(ProfileGuard::new_owned("Collapse"))
                    } else {
                        None
                    };
                    let val_rs = state.stack.pop().ok_or("VM stack underflow on Collapse")?;
                    // substitute environment variables on collapse
                    let atoms: Vec<Atom> = val_rs
                        .into_iter()
                        .map(|(a, env)| subst_atom(&a, &env))
                        .collect();
                    state.stack.push(plain(vec![Atom::Expr(expr_data(atoms))]));
                    state.ip += 1;
                }
                Opcode::Test => {
                    let _profile = if cfg!(feature = "profile") {
                        Some(ProfileGuard::new_owned("Test"))
                    } else {
                        None
                    };
                    // Pop expected (evaluated second, so on top)
                    let expected_rs = state
                        .stack
                        .pop()
                        .ok_or("VM stack underflow on Test expected")?;
                    ...
                    let expr_atoms: Vec<Atom> = expr_rs
                        .into_iter()
                        .map(|(a, env)| subst_atom(&a, &env))
                        .collect();
                    ...
                    let expected = if expected_atoms.len() == 1 {
                        ...
                        Atom::Expr(expr_data(expected_atoms))
                    };
                    ...
                    eprintln!(
                        "is {}, should {}. {} ",
                        actual.to_sexpr_string(),
                        expected.to_sexpr_string(),
                        emoji
                    );
                ...
                Opcode::Superpose(count) => {
                    let _profile = if cfg!(feature = "profile") {
                        Some(ProfileGuard::new_owned("Superpose"))
                    } else {
                        None
                    };
                    let mut results = Vec::new();
                ...
                Opcode::SuperposeUnpack => {
                    let _profile = if cfg!(feature = "profile") {
                        Some(ProfileGuard::new_owned("SuperposeUnpack"))
                    } else {
                        None
                    };
                    let val_rs = state
                        .stack
                        .pop()
                        .ok_or("VM stack underflow on SuperposeUnpack")?;
                ...
                Opcode::ConstEmpty => {
                    let _profile = if cfg!(feature = "profile") {
                        Some(ProfileGuard::new_owned("ConstEmpty"))
                    } else {
                        None
                    };
                    state.stack.push(Vec::new());
                ...
                Opcode::Println => {
                    ...
                    let res = finish_println(arg_rs);
                    state.stack.push(res);
                    state.ip += 1;
                }
                Opcode::Readln => {
                    let _profile = if cfg!(feature = "profile") {
                        Some(ProfileGuard::new_owned("Readln"))
                    } else {
                        None
                    };
                    let nd = eval_readln(&[], &base_env, funcs)?;
                ...
                Opcode::Match {
                    ...
                    let _profile = if cfg!(feature = "profile") {
                        Some(ProfileGuard::new_owned("Match"))
                    } else {
                        None
                    };
                    ...
                    let mut resume = match state
                        .resume_data
                        .take()
                        .and_then(|r| r.downcast::<MatchResume>().ok())
                    {
                        Some(r) => {
                            let mut r = *r;
                            if let Some((sub_res, sub_exit)) = state.last_sub_result.take() {
                        ...
                        None => {
                            let space_rs = state
                                .stack
                                .pop()
                                .ok_or("VM stack underflow on Match space")?;
                            let mut results = Vec::new();
                            ...
                            if let Some((space_ref, _)) = space_rs.first() {
                                ...
                                if let Ok(matches) =
                                    collect_match_results(funcs, space_ref, pattern, &match_env)
                                {
                                    if !matches.is_empty() {
                                        let precomputed_bindings: Vec<Atom> = free_vars_map
                                            .iter()
                                        ...
                                        for matched in matches {
                                            ...
                                            let sub_env = bind_all(&match_env, &matched.bindings);
                ...
                Opcode::Eval => {
                    let _profile = if cfg!(feature = "profile") {
                        Some(ProfileGuard::new_owned("Eval"))
                    } else {
                        None
                    };
                ...
                Opcode::If {
                    ...
                    let _profile = if cfg!(feature = "profile") {
                        Some(ProfileGuard::new_owned("If"))
                    } else {
                        None
                    };
                ...
                Opcode::Let {
                    ...
                    let _profile = if cfg!(feature = "profile") {
                        Some(ProfileGuard::new_owned("Let"))
                    } else {
                        None
                    };
                ...
                Opcode::Foldall => {
                    ...
                    let agg_rs = state
                        .stack
                        .pop()
                        .ok_or("VM stack underflow on Foldall agg-func")?;
                    let init_rs = state
                        .stack
                        .pop()
                        .ok_or("VM stack underflow on Foldall init")?;
                    let gen_rs = state
                        .stack
                        .pop()
                        .ok_or("VM stack underflow on Foldall generator")?;
                    ...
                    let init_atom = init_rs
                        .into_iter()
                        .next()
                        .map(|(a, env)| subst_atom(&a, &env))
                        .ok_or_else(|| "foldall: init produced no result".to_string())?;
                    let gen_values: Vec<Atom> = gen_rs
                        .into_iter()
                        .map(|(a, env)| subst_atom(&a, &env))
                        .collect();

                    let (agg_head, agg_env) = match &agg_atom {
                        Atom::Sym(name) => (Expr::Symbol(name.to_string()), base_env.clone()),
                        Atom::Closure(_) => (
                            Expr::Symbol("$__foldall_fn".to_string()),
                            bind(&base_env, "$__foldall_fn", agg_atom.clone()),
                        ),
                        _ => {
                            return Err("foldall: agg-func must be a function symbol or closure"
                                .to_string());
                        }
                    };
                    ...
                    for val in gen_values {
                        let acc_expr = atom_to_expr(&accum)?;
                ...
                Opcode::Forall => {
                    let _profile = if cfg!(feature = "profile") {
                        Some(ProfileGuard::new_owned("Forall"))
                    } else {
                        None
                    };
                    ...
                    let check_rs = state
                        .stack
                        .pop()
                        .ok_or("VM stack underflow on Forall check")?;
                    let gen_rs = state
                        .stack
                        .pop()
                        .ok_or("VM stack underflow on Forall generator")?;

                    // substitute env
                    let check_atom = check_rs
                        .into_iter()
                        .next()
                        .map(|(a, env)| subst_atom(&a, &env))
                    ...
                    let check_head = match &check_atom {
                        ...
                        _ => {
                            return Err(
                                "forall: check must be a function symbol or closure".to_string()
                            );
                        }
                    };
                    ...
                    let is_forall_true = gen_values.into_par_iter().all(|val| {
                        ...
                        let mut call_env = bind(&base_env, "$__fv", val);
                        if let Atom::Closure(_) = &check_atom {
                            let check_env = bind(&base_env, "$__check_fn", check_atom.clone());
                            call_env = prepend_env(check_env, &call_env);
                        }
                        if let Ok(res) = eval_call_vm(
                            ...
                            let results: Vec<Atom> = res
                                .into_iter()
                                .map(|(a, env)| subst_atom(&a, &env))
                                .collect();
                            !results.is_empty() && results.iter().all(|a| is_truthy(a))
                        } else {
                            false
                        }
                    });

                    let final_atom = if is_forall_true {
                        Atom::sym("true")
                    } else {
                        Atom::sym("false")
                    };
                ...
                Opcode::Foldl => {
                    ...
                    let func_rs = state
                        .stack
                        .pop()
                        .ok_or("VM stack underflow on Foldl func")?;
                    let acc_rs = state.stack.pop().ok_or("VM stack underflow on Foldl acc")?;
                    let list_rs = state
                        .stack
                        .pop()
                        .ok_or("VM stack underflow on Foldl list")?;
                    ...
                    let items: Vec<Atom> = match list_rs
                        .into_iter()
                        .next()
                        .map(|(a, env)| subst_atom(&a, &env))
                    {
                    ...
                    for item in items {
                        let acc_expr = atom_to_expr(&current_acc)
                            .unwrap_or_else(|_| Expr::Symbol(current_acc.to_sexpr_string()));
                        let item_expr = atom_to_expr(&item)
                ...
                Opcode::FoldlLambda {
                    ...
                    let mut resume = match state
                        .resume_data
                        .take()
                        .and_then(|r| r.downcast::<FoldlLambdaResume>().ok())
                    {
                        Some(r) => {
                            let mut r = *r;
                            if let Some((sub_res, sub_exit)) = state.last_sub_result.take() {
                                r.current_acc = sub_res
                                    .into_iter()
                                    .next()
                                    .map(|(a, env)| subst_atom(&a, &env))
                        ...
                        None => {
                            let acc_rs = state
                                .stack
                                .pop()
                                .ok_or("VM stack underflow on FoldlLambda acc")?;
                            let list_rs = state
                                .stack
                                .pop()
                                .ok_or("VM stack underflow on FoldlLambda list")?;
                            let acc = acc_rs
                                .into_iter()
                                .next()
                                .map(|(a, env)| subst_atom(&a, &env))
                                .ok_or_else(|| {
                                    "foldl-atom: acc arg produced no result".to_string()
                                })?;
                            let items: Vec<Atom> = match list_rs
                                .into_iter()
                                .next()
                                .map(|(a, env)| subst_atom(&a, &env))
                            {
                                ...
                                None => {
                                    return Err(
                                        "foldl-atom: list arg produced no result".to_string()
                                    );
                                }
                            };
                            FoldlLambdaResume {
                ...
                Opcode::MapAtomLambda {
                    ...
                    let _profile = if cfg!(feature = "profile") {
                        Some(ProfileGuard::new_owned("MapAtomLambda"))
                    } else {
                        None
                    };
                    ...
                    let list_rs = state
                        .stack
                        .pop()
                        .ok_or("VM stack underflow on MapAtomLambda")?;
                    let items: Vec<Atom> = match list_rs
                        .into_iter()
                        .next()
                        .map(|(a, env)| subst_atom(&a, &env))
                    ...
                    let mapped: Result<Vec<Atom>, String> = items
                        .into_par_iter()
                        .map(|elem| {
                            let mut sub_state = VMState::new_with_parent(
                                body_code.clone(),
                    ...
                    state
                        .stack
                        .push(plain(vec![Atom::Expr(expr_data(mapped?))]));
                    state.ip += 1;
                }
                Opcode::FilterAtomLambda {
                    ...
                    let _profile = if cfg!(feature = "profile") {
                        Some(ProfileGuard::new_owned("FilterAtomLambda"))
                    } else {
                        None
                    };
                    ...
                    let list_rs = state
                        .stack
                        .pop()
                        .ok_or("VM stack underflow on FilterAtomLambda")?;
                    let items: Vec<Atom> = match list_rs
                        .into_iter()
                        .next()
                        .map(|(a, env)| subst_atom(&a, &env))
                    ...
                    let mapped: Result<Vec<Option<Atom>>, String> = items
                        .into_par_iter()
                        .map(|elem| {
                            let mut sub_state = VMState::new_with_parent(
                                body_code.clone(),
                            ...
                            let is_true = res
                                .into_iter()
                                .next()
                                .map(|(a, env)| subst_atom(&a, &env).is_truthy())
                                .unwrap_or(false);
                            if is_true { Ok(Some(elem)) } else { Ok(None) }
                        })
                        .collect();
                    ...
                    state
                        .stack
                        .push(plain(vec![Atom::Expr(expr_data(filtered))]));
                    state.ip += 1;
                }
                Opcode::Once {
                    body_code,
                    ...
                    struct OnceResume;
                    let (res, exit_status) = match state
                        .resume_data
                        .take()
                        .and_then(|r| r.downcast::<OnceResume>().ok())
                    {
                        Some(_) => {
                            let (r, e) = state
                                .last_sub_result
                                .take()
                                .unwrap_or_else(|| (Vec::new(), VmExit::Normal));
                            (r, e)
                        }
                        None => {
                            let mut sub_state = VMState::new_with_parent(
                ...
                Opcode::Progn {
                    ...
                    let mut resume = match state
                        .resume_data
                        .take()
                        .and_then(|r| r.downcast::<PrognResume>().ok())
                    {
                        Some(r) => {
                            let mut r = *r;
                            if let Some((sub_res, sub_exit)) = state.last_sub_result.take() {
                ...
                Opcode::Prog1 {
                    ...
                    let mut resume = match state
                        .resume_data
                        .take()
                        .and_then(|r| r.downcast::<Prog1Resume>().ok())
                    {
                        Some(r) => {
                            let mut r = *r;
                            if let Some((sub_res, sub_exit)) = state.last_sub_result.take() {
                ...
                Opcode::Chain {
                    ...
                    let mut resume = match state
                        .resume_data
                        .take()
                        .and_then(|r| r.downcast::<ChainResume>().ok())
                    {
                        Some(r) => {
                            let mut r = *r;
                            if let Some((sub_res, sub_exit)) = state.last_sub_result.take() {
                                ...
                                if is_final {
                                    ...
                                    let val = val_opt.ok_or_else(|| {
                                        format!("chain: step {} produced no result", var_name)
                                    })?;
                                    if let Some(pos) =
                                        state.free_vars_map.iter().position(|x| x == var_name)
                                    {
                ...
                Opcode::Within {
                    ...
                    let (res, exit_status) = match state
                        .resume_data
                        .take()
                        .and_then(|r| r.downcast::<WithinResume>().ok())
                    {
                        Some(_) => {
                            let (r, e) = state
                                .last_sub_result
                    ...
                    let wrapped = Atom::Expr(expr_data(
                ...
                Opcode::WithMutex {
                    ...
                    struct WithMutexResume {
                        mutex_name: String,
                    }
                    let (res, exit_status) = match state
                        .resume_data
                        .take()
                        .and_then(|r| r.downcast::<WithMutexResume>().ok())
                    {
                        Some(_) => {
                            let (r, e) = state
                                .last_sub_result
                        ...
                        None => {
                            let name_rs =
                                state.stack.pop().ok_or("VM stack underflow on WithMutex")?;
                            let mutex_name = name_rs
                                .into_iter()
                                .next()
                                .map(|(a, env)| subst_atom(&a, &env).to_sexpr_string())
                                .unwrap_or_default();
                            let mut sub_state = VMState::new_with_parent(
                                body_code.clone(),
                                free_vars_map.clone(),
                            ...
                            let result = with_named_mutex(&mutex_name, || {
                ...
                Opcode::Transaction {
                    ...
                    let (res, exit_status) = match state
                        .resume_data
                        .take()
                        .and_then(|r| r.downcast::<TransactionResume>().ok())
                    {
                        Some(r) => {
                            let r = *r;
                            let (res, e) = state
                            ...
                            match &e {
                                VmExit::TailCall(_) => {
                                    restore_transaction_state(r.snapshot, funcs).map_err(|e| {
                ...
                Opcode::ImportFile { path } => {
                    ...
                    let space_rs = state
                        .stack
                        .pop()
                        .ok_or("VM stack underflow on ImportFile")?;
                    // substitute env
                    ...
                    let resolved = resolve_import_path(&path, &import_dir).ok_or_else(|| {
                        format!(
                    ...
                    let new_dir = resolved
                        ...
                        .to_path_buf();
                    let prev_dir =
                        std::mem::replace(&mut *funcs.import_dir.lock().unwrap(), new_dir);
                    let atoms = load_metta_file(&resolved, &space_ref, &base_env, funcs)?;
                ...
                Opcode::PythonImport { path } => {
                    ...
                    eval_py_import_library(&resolved)?;
                    state.stack.push(plain(vec![Atom::sym("true")]));
                    state.ip += 1;
                }
                Opcode::PyCall { expr } => {
                    ...
                    let result =
                        crate::eval::python::eval_py_call(&[expr.clone()], &base_env, funcs)?;
                    let atoms: Vec<Atom> = result.collect();
                    state.stack.push(plain(atoms));
                    state.ip += 1;
                }
                Opcode::PyEval { expr } => {
                    match eval_py_eval(&[expr.clone()], &base_env, funcs) {
                        Ok(nd) => {
                            state.stack.push(plain(nd.collect()));
                        }
                        Err(e) => return Err(e),
                ...
                Opcode::ImportDynamic => {
                    let path_rs = state
                        .stack
                        .pop()
                        .ok_or("VM stack underflow on ImportDynamic path")?;
                    let space_rs = state
                        .stack
                        .pop()
                        .ok_or("VM stack underflow on ImportDynamic space")?;

                    // substitute env
                    let path = match path_rs.first().map(|(a, env)| subst_atom(&a, &env)) {
                        Some(Atom::Sym(s)) => std::sync::Arc::from(s.as_str()),
                        Some(Atom::Str(s)) => s.clone(),
                        Some(Atom::Expr(expr)) if expr.len() == 2 => {
                            if let (Some(Atom::Sym(head)), Some(py_atom)) =
                    ...
                    if is_py {
                        ...
                        let import_dir = funcs.import_dir.lock().unwrap().clone();
                        let resolved =
                            resolve_import_path(path.as_ref(), &import_dir).ok_or_else(|| {
                                format!(
                                    "import!: cannot find '{}' (searched CWD and '{}')",
                                    path,
                                    import_dir.display()
                ...
                Opcode::MapAtomPatternLambda {
                    ...
                    let mut resume = match state
                        .resume_data
                        .take()
                        .and_then(|r| r.downcast::<MapAtomPatternResume>().ok())
                    {
                        Some(r) => {
                            let mut r = *r;
                            if let Some((sub_res, sub_exit)) = state.last_sub_result.take() {
                                if let Some((val, env)) = sub_res.into_iter().next() {
                                    r.mapped_results.push(subst_atom(&val, &env));
                                }
                                match sub_exit {
                                    VmExit::Cut => {
                                        state.cut_executed = true;
                                    ...
                                    VmExit::TailCall(locs) => {
                                        return Ok((
                                            plain(vec![Atom::Expr(expr_data(r.mapped_results))]),
                                            state.budget,
                                            VmExit::TailCall(locs),
                                        ));
                                    }
                        ...
                        None => {
                            let list_rs = state
                                .stack
                                .pop()
                                .ok_or("VM stack underflow on MapAtomPatternLambda")?;
                            let items: Vec<Atom> = match list_rs
                                .into_iter()
                                .next()
                                .map(|(a, env)| subst_atom(&a, &env))
                            {
                                ...
                                Some(other) => vec![other],
                                None => {
                                    return Err("map-atom: list arg produced no result".to_string());
                                }
                            };
                            let cap = items.len();
                            MapAtomPatternResume {
                                items,
                                next_item_idx: 0,
                                mapped_results: Vec::with_capacity(cap),
                            }
                        }
                    };
                    while resume.next_item_idx < resume.items.len() && !state.cut_executed {
                        let elem = resume.items[resume.next_item_idx].clone();
                        resume.next_item_idx += 1;
                        let matched = try_match_one(pattern, &elem, &base_env, funcs)?;
                    ...
                    state
                        .stack
                        .push(plain(vec![Atom::Expr(expr_data(resume.mapped_results))]));
                    state.ip += 1;
                }
                Opcode::FilterAtomPatternLambda {
                    pattern,
                    ...
                    let mut resume = match state
                        .resume_data
                        .take()
                        .and_then(|r| r.downcast::<FilterAtomPatternResume>().ok())
                    {
                        Some(r) => {
                            let mut r = *r;
                            if let Some((sub_res, sub_exit)) = state.last_sub_result.take() {
                                ...
                                if is_true {
                                    r.filtered_results
                                        .push(r.items[r.next_item_idx - 1].clone());
                                }
                                match sub_exit {
                                    VmExit::Cut => {
                                        state.cut_executed = true;
                                    ...
                                    VmExit::TailCall(locs) => {
                                        return Ok((
                                            plain(vec![Atom::Expr(expr_data(r.filtered_results))]),
                                            state.budget,
                                            VmExit::TailCall(locs),
                                        ));
                                    }
                        ...
                        None => {
                            let list_rs = state
                                .stack
                                .pop()
                                .ok_or("VM stack underflow on FilterAtomPatternLambda")?;
                            let items: Vec<Atom> = match list_rs
                                .into_iter()
                                .next()
                                .map(|(a, env)| subst_atom(&a, &env))
                            {
                                ...
                                None => {
                                    return Err(
                                        "filter-atom: list arg produced no result".to_string()
                                    );
                                }
                            };
                            let cap = items.len();
                            FilterAtomPatternResume {
                                items,
                                next_item_idx: 0,
                                filtered_results: Vec::with_capacity(cap),
                            }
                        }
                    };
                    while resume.next_item_idx < resume.items.len() && !state.cut_executed {
                    ...
                    state
                        .stack
                        .push(plain(vec![Atom::Expr(expr_data(resume.filtered_results))]));
                    state.ip += 1;
                }
                Opcode::TailCallSelf => {
                        ...
                        if let Some(c_idx) = call_frame_idx {
                            ...
                            if let Some(clauses) = lookup_user_clauses(name, arity, funcs) {
                                ...
                                let cached = FN_BYTECODE_CACHE.with(|cache_ref| {
                                    Ok::<_, String>(
                                        cache_ref.borrow().get(&cache_key).unwrap().clone(),
                                    )
                                })?;

                                for (i, (patterns, body)) in clauses.iter().enumerate() {
                                    if let Some((body_env, subst_cost)) =
                                        match_clause(patterns, &new_args, &base_env, funcs)
                                    {
                                        let clause = &cached[i];
                                        let body_cost = calculate_expr_cost(body);
                                        let total_cost = subst_cost + body_cost;

                                        let mut locals_to_push =
                                            Vec::with_capacity(clause.locals.len());
        ...
        if state.ip >= state.code.len() || state.cut_executed {
            if let Some(frame) = state.frames.last_mut() {
                ...
                if matches!(
                    frame.kind,
                    CallFrameKind::Call { .. } | CallFrameKind::Eval { .. }
                ) {
                ...
                match &mut frame.kind {
                    CallFrameKind::Let { results, .. } => {
                        results.extend(sub_results);
                    }
                    CallFrameKind::If { results, .. } => {
                        results.extend(sub_results);
                    }
                    CallFrameKind::Case { results, .. } => {
                        results.extend(sub_results);
                    }
                    CallFrameKind::Call { results, .. } => {
                        let merged: Vec<(Atom, Env)> = sub_results
                            .into_iter()
                            .map(|(atom, env)| {
                                let merged_env = prepend_chain(env, &base_env);
                                (atom, merged_env)
                            })
                            .collect();
    ...
    let prepended_rs: Vec<(Atom, Env)> = final_rs
        .into_iter()
        .map(|(atom, env)| {
            let merged = prepend_chain(env, &base_env);
            (atom, merged)
        })
        .collect();
    let exit = if state.cut_executed {
        VmExit::Cut
    } else {
        VmExit::Normal
    };
...
fn eval_call_vm(
    ...
    if comp.compile(&call, &mut code, false).is_ok() {
        ...
        *budget = sub_budget;
        ...
        run_rs(Arc::new(call), env.clone(), funcs, budget)
    }
}
...
fn dispatch_call(
    ...
    match head_atom {
        Atom::Sym(fn_name) => {
            if let Some(function) = funcs.get(fn_name, arity) {
                match &function.kind {
                    FunctionKind::Native { func: native_f } => {
                        let _profile = if cfg!(feature = "profile") {
                            Some(ProfileGuard::new_owned(fn_name))
                        } else {
                            None
                        };
                        ...
                        if fn_name.as_ref() == "member" && arity == 2 {
                            if let Atom::Sym(ref s) = args[1] {
                                if s.starts_with('$') && combo_env.get(s).is_none() {
                                    ...
                                    let list_val = Atom::Expr(expr_data(vec![
                                        args[0].clone(),
                                        Atom::sym(&fresh_tail),
                                    ]));
                                    let mut merged_env = combo_env.clone();
                                    merged_env = bind(&merged_env, s, list_val);
                            ...
                            for item in &items {
                                ...
                                } else if let Some(bindings) = unify(&args[0], item) {
                                    let mut merged_env = combo_env.clone();
                                    for (name, val) in bindings {
                                        merged_env = bind(&merged_env, &name, val);
                                    }
                                    results.push((Atom::sym("True"), merged_env));
                                }
                            }
                        ...
                        if fn_name.as_ref() == "is-member" && arity == 2 {
                            if let Atom::Sym(ref s) = args[1] {
                                if s.starts_with('$') && combo_env.get(s).is_none() {
                                    ...
                                    let mut merged_env = combo_env.clone();
                                    merged_env = bind(&merged_env, s, list_val);
                                    results.push((bool_atom(true), merged_env));
                                    return Ok(());
                                }
                            }
                            let items = match &args[1] {
                            ...
                            for item in &items {
                                if args[0] == *item {
                                    unifiable_results.push((bool_atom(true), combo_env.clone()));
                                } else if let Some(bindings) = unify(&args[0], item) {
                                    let mut merged_env = combo_env.clone();
                                    for (name, val) in bindings {
                                        merged_env = bind(&merged_env, &name, val);
                                    }
                                    unifiable_results.push((bool_atom(true), merged_env));
                                }
                            }
                            if unifiable_results.is_empty() {
                                results.push((bool_atom(false), combo_env.clone()));
                            } else {
            ...
            let mut matched_any = false;
            let user_clauses = lookup_user_clauses(fn_name, arity, funcs);
            ...
            if let Some(clauses) = user_clauses {
                ...
                let cached = FN_BYTECODE_CACHE.with(|cache_ref| {
                    ...
                    for (pat, b) in &clauses {
                        let mut comp =
                            super::compiler::VMCompiler::new(pat, Some(fn_name.to_string()));
                        let mut code = Vec::new();
                ...
                for (i, (patterns, body)) in clauses.iter().enumerate() {
                    if let Some((body_env, subst_cost)) =
                        match_clause(patterns, args, combo_env, funcs)
                    {
                        matched_any = true;
                        let clause = &cached[i];
                        let body_cost = calculate_expr_cost(body);
            ...
            if !matched_any {
                if funcs.has_greater_arity(fn_name, arity) {
                    let partial_atom = Atom::Expr(expr_data(vec![
                        Atom::sym("partial"),
                        head_atom.clone(),
                        Atom::Expr(expr_data(args.to_vec())),
                    ]));
                    ...
                    let substituted: Vec<Atom> =
                        items.iter().map(|a| subst_atom(a, combo_env)).collect();
                    results.push((Atom::Expr(expr_data(substituted)), combo_env.clone()));
                }
            }
        }
        Atom::Closure(c) => {
            ...
            for (patterns, body) in &clauses {
                if let Some((body_env, _subst_cost)) =
                    match_clause(patterns, args, combo_env, funcs)
                {
                    matched_any = true;
                    let body_renamed = rename_apart_unbound_vars(body, patterns);
        ...
        _ => {
            let mut items = vec![head_atom.clone()];
            items.extend(args.to_vec());
            let substituted: Vec<Atom> = items.iter().map(|a| subst_atom(a, combo_env)).collect();
            results.push((Atom::Expr(expr_data(substituted)), combo_env.clone()));
        }
    }
    Ok(())
...
fn run_next_let_iteration(state: &mut VMState, funcs: &FnTable) -> Result<Option<Env>, String> {
    ...
    if let Some(frame) = state.frames.last_mut() {
        ...
        } = &mut frame.kind
        {
            while *next_idx < value_rs.len() {
                ...
                *next_idx += 1;
                if let Some(match_env) = try_match_one(pattern, value, &Env::new(), funcs)? {
                    let body_env = prepend_chain(match_env, value_env);
    ...
    if let Some((body_code, free_vars_map, body_env, locals_to_push)) = to_run {
        ...
        for (val, env) in &mut state.locals {
            let substituted = subst_atom(val, &body_env);
            *val = substituted;
            *env = prepend_chain(body_env.clone(), env);
        }
...
fn run_next_if_iteration(state: &mut VMState) -> Result<Option<Env>, String> {
        ...
        } = &mut frame.kind
        {
            while *next_idx < condition_rs.len() {
                ...
                *next_idx += 1;
                ...
                if is_true {
                    let branch_env = prepend_env(cond_env.clone(), &frame.saved_base_env);
                    to_run = Some((
                        then_code.clone(),
                        free_vars_map.clone(),
                        branch_env,
    ...
    if let Some((code_to_run, free_vars_map, branch_env, cond_env_opt)) = to_run {
        ...
        if let Some(ref cond_env) = cond_env_opt {
            for (val, env) in &mut state.locals {
                let substituted = subst_atom(val, cond_env);
                *val = substituted;
                *env = prepend_chain(cond_env.clone(), env);
            }
        }
        ...
        let frame = state.frames.pop().unwrap();
        if let CallFrameKind::If { results, .. } = frame.kind {
            state.code = frame.return_code;
            state.ip = frame.return_ip + 1;
            state.free_vars_map = frame.saved_free_vars_map;
            state.free_vars_bindings = frame.saved_free_vars_bindings;
                ...
                while *next_idx < scrutinee_rs.len() {
                    ...
                    *next_idx += 1;
                    ...
                    for branch in branches {
                        ...
                        if let Some(match_env) =
                            try_match_one(&branch.pattern, value, &Env::new(), funcs)?
                        {
                            let body_env = prepend_chain(match_env, value_env);
...
fn run_call_body(
    ...
    let free_vars_bindings: Vec<Atom> = pending
        .free_vars
        .iter()
        .map(|name| {
...
fn run_next_call_iteration(state: &mut VMState, funcs: &FnTable) -> Result<Option<Env>, String> {
    ...
    let (all_pure, pending_len) = {
        ...
        } = kind
        {
            ...
            (
                remaining > 0 && pending_calls[*next_idx..].iter().all(|p| p.pure),
                remaining,
            )
        } else {
            return Ok(None);
        ...
        *next_idx = pending_calls.len(); // consume all
        ...
        if let CallFrameKind::Call { memo_key, .. } = frame.kind {
            if let Some(key) = memo_key {
                let atoms_only: Vec<Atom> =
                    results.iter().map(|(a, env)| subst_atom(a, env)).collect();
            ...
            while *next_idx < target_rs.len() {
                ...
                *next_idx += 1;
                ...
                if comp.compile(target_expr, &mut code, false).is_ok() {
                    to_run = Some((
                        std::sync::Arc::from(code),
                        Arc::from(comp.free_vars),
                        target_env.clone(),
                    ));
                    break;
                } else {
                    let mut budget = state.budget;
                    let res = super::super::step::run_rs(
                        Arc::new(target_expr.clone()),
                        target_env.clone(),
                        funcs,
                        &mut budget,
                    )?;
