...
pub fn register_state_builtins(funcs: &FnTable) {
    funcs.insert_native("get-atoms", 1, |args, table| {
        crate::builtins::arithmetic::expect_n_args(args, 1, "get-atoms")?;
    ...
    funcs.insert_native("change-state!", 2, |args, table| {
        crate::builtins::arithmetic::expect_n_args(args, 2, "change-state!")?;
    ...
    funcs.insert_native("get-state", 1, |args, table| {
        crate::builtins::arithmetic::expect_n_args(args, 1, "get-state")?;
        let key = match &args[0] {
            Atom::Sym(s) => s.to_string(),
    ...
    funcs.insert_native("bind!", 2, |args, table| {
        crate::builtins::arithmetic::expect_n_args(args, 2, "bind!")?;
        let key = match &args[0] {
            Atom::Sym(s) => s.to_string(),
            other => return Err(format!("bind!: key must be a symbol, got {}", other.to_sexpr_string())),
        };
        let value = match &args[1] {
            Atom::Expr(items) if items.len() == 2 && items[0] == Atom::sym("new-state") => items[1].clone(),
            other => other.clone(),
        };
